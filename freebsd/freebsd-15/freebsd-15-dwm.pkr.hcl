...
variable "iso_checksum" {
  type        = string
  default     = "file:https://download.freebsd.org/releases/ISO-IMAGES/15.0/CHECKSUM.SHA256-FreeBSD-15.0-RELEASE-amd64"
  description = "SHA256 checksum of the install media."
}

variable "iso_name" {
  type        = string
  default     = "FreeBSD-15.0-RELEASE-amd64-dvd1.iso"
  description = "File name of the install media."
}

variable "iso_path" {
  type        = string
  default     = "releases/ISO-IMAGES/15.0"
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      "VIRTUALBOX_OSE_ADDITIONS=virtualbox-ose-additions-6.1.50.1500068_1"
    ]
    execute_command = "chmod +x {{ .Path }}; env {{ .Vars }} {{ .Path }}"
    only = [
