...
variable "iso_checksum" {
  type        = string
  default     = "file:https://download.freebsd.org/releases/ISO-IMAGES/15.1/CHECKSUM.SHA256-FreeBSD-15.1-BETA1-amd64"
  description = "SHA256 checksum of the install media."
}

variable "iso_name" {
  type        = string
  default     = "FreeBSD-15.1-BETA1-amd64-dvd1.iso"
  description = "File name of the install media."
}

variable "iso_path" {
  type        = string
  default     = "releases/ISO-IMAGES/15.1"
...
variable "ncurses_version" {
  type        = string
  default     = "6.6"
  description = "Version of `ncurses` package."
}
...
variable "xorg_fonts" {
  type        = string
  default     = "xorg-fonts-7.7_1"
  description = "Version of `xorg-fonts` package."
}
...
locals {
  ...
  bsdinstall_distsite = "${var.dist_site}/${replace(var.arch, "aarch64", "arm64/aarch64")}/${var.os_version}/"
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      ...
      "XF86_VIDEO=xf86-video-vesa-2.5.0_3"
    ]
    execute_command = "chmod +x {{ .Path }}; env {{ .Vars }} {{ .Path }}"
    only = [
      "qemu.default"
  ...
  provisioner "shell" {
    environment_vars = [
      "VIRTUALBOX_OSE_ADDITIONS=virtualbox-ose-additions-6.1.50.1500068_3"
    ]
    execute_command = "chmod +x {{ .Path }}; env {{ .Vars }} {{ .Path }}"
    only = [
  ...
  provisioner "shell" {
    environment_vars = [
      ...
      "DMENU=dmenu-5.1",
      "DWM=dwm-6.8",
      "NCURSES=ncurses-${var.ncurses_version}",
      "STERM=sterm-0.9.3",
      "TERMINFO_DB=terminfo-db-20250719",
      "XORG_FONTS=${var.xorg_fonts}",
      "XORG_MINIMAL=xorg-minimal-7.5.2_3",
      "XORGXRDP=xorgxrdp-0.10.5",
      "XRANDR=xrandr-1.5.3",
      "XRDP=xrdp-${var.xrdp_version}"
    ]
    execute_command = "chmod +x {{ .Path }}; env {{ .Vars }} {{ .Path }}"
    scripts = [
      "../provisioners/dwm+dmenu+st_freebsd15.1.sh",
