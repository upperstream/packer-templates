...
variable "iso_checksum" {
  type    = string
  default = "file:https://download.freebsd.org/releases/ISO-IMAGES/13.3/CHECKSUM.SHA256-FreeBSD-13.3-RELEASE-amd64"
}

variable "iso_image" {
  type    = string
  default = "FreeBSD-13.3-RELEASE-amd64-disc1.iso"
}
  ...
  http_content = {
    "/install.sh" = templatefile("${path.root}/install.sh.pkrtpl.hcl", {
      "variables" = {
        ...
        "export ABI"              = var.ABI
        "export CA_ROOT_NSS"      = "ca_root_nss-3.93_2"
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      "OPEN_VM_TOOLS=open-vm-tools-12.3.5_1,2",
      "VMWARE_WITH_XORG=1",
