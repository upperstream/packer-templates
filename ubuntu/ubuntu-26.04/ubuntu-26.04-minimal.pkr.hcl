...
build {
  ...
  provisioner "shell" {
    ...
    execute_command = "echo '${var.vagrant_password}' | {{ .Vars }} sudo --preserve-env=DEBIAN_FRONTEND,USE_VIRTUALBOX_UBUNTU_PACKAGE,VAGRANT_USERNAME,VIRTUALBOX_VERSION -S sh -ex '{{ .Path }}'"
    only = [
