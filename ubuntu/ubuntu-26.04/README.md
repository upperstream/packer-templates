# Packer templates for Ubuntu 26.04 LTS

Templates to create Vagrant boxes for Ubuntu 26.04 LTS (amd64 and
arm64).
...
From the terminal, invoke the following command for VirtualBox provider:
    ...
    packer build -only=virtualbox-iso.default ubuntu-26.04-minimal.pkr.hcl

You will find a vagrant box file named
`Ubuntu-26.04-minimal-v2604.0.20260423-amd64-virtualbox.box` in the
same directory after the command has succeeded.

Then you can add the box named `Ubuntu-26.04-minimal-v2604.0.20260423`
to your box list by the following command:

    vagrant box add Ubuntu-26.04-minimal-v2604.0.20260423-amd64-virtualbox.box \
      --name Ubuntu-26.04-minimal-v2604.0.20260423 --provider virtualbox
...
From the terminal, invoke the following command for VMware provider:
    ...
    packer build -only=vmware-iso.default ubuntu-26.04-minimal.pkr.hcl

You will find a vagrant box file named
`Ubuntu-26.04-minimal-v2604.0.20260423-amd64-vmware.box` in the same
directory after the command has succeeded.

Then you can add the box named `Ubuntu-26.04-minimal-v2604.0.20260423`
to your box list by the following command:

    vagrant box add Ubuntu-26.04-minimal-v2604.0.20260423-amd64-vmware.box \
      --name Ubuntu-26.04-minimal-v2604.0.20260423 --provider vmware_desktop
...
The following command will build a VM image on your ESXi:
    ...
    packer build -only=vmware-iso.esxi ubuntu-26.04-minimal.pkr.hcl

When you create a box on ESXi host version prior to 6.7, you need to
enable VNC on the host and need to disable Packer's VNC over WebSocket
feature by adding `-var esxi_vnc_over_websocket=false` parameter:

    packer build -only=vmware-iso.esxi -var esxi-vnc-over-websocket=false ubuntu-26.04-minimal.pkr.hcl

### QEMU/libvirt

From the terminal, invoke the following command for Libvirt provider:

    packer build -only=qemu.default ubuntu-26.04-minimal.pkr.hcl

You will find a vagrant box file named
`Ubuntu-26.04-minimal-v2604.0.20260423-amd64-libvirt.box` in the same
directory after the command has succeeded.

Then you can add the box named `Ubuntu-26.04-minimal-v2604.0.20260423`
to your box list by the following command:

    vagrant box add Ubuntu-26.04-minimal-v2604.0.20260423-amd64-libvirt.box \
      --name Ubuntu-26.04-minimal-v2604.0.20260423 --provider libvirt
...
From the terminal, invoke the following command for Hyper-V provider:
    ...
    packer build -only=hyperv-iso.default ubuntu-26.04-minimal.pkr.hcl

You will find a vagrant box file named
`Ubuntu-26.04-minimal-v2604.0.20260423-amd64-hyperv.box` in the same
directory after the command has succeeded.

Then you can add the box named `Ubuntu-26.04-minimal-v2604.0.20260423`
to your box list by the following command:

    vagrant box add Ubuntu-26.04-minimal-v2604.0.20260423-amd64-hyperv.box \
      --name Ubuntu-26.04-minimal-v2604.0.20260423 --provider hyperv
...
From the terminal, invoke the following command for Parallels provider:
    ...
    packer build -only=parallels-iso.default \
      -var-file vars-ubuntu-26.04-arm64.pkrvars.hcl ubuntu-26.04-minimal.pkr.hcl

You will find a vagrant box file named
`Ubuntu-26.04-minimal-v2604.0.20260423-arm64-parallels.box`
in the same directory after the command has succeeded.

Then you can add the box named `Ubuntu-26.04-minimal-v2604.0.20260423`
to your box list by the following command:

    vagrant box add Ubuntu-26.04-minimal-v2604.0.20260423-arm64-parallels.box \
      --name Ubuntu-26.04-minimal-v2604.0.20260423 --provider parallels

Parallels build intends to create arm64 box on Apple Silicon Mac host.

## Variants

* `ubuntu-26.04-minimal.pkr.hcl` - Ubuntu Server 26.04 LTS
* `ubuntu-26.04-dwm.pkr.hcl` - Ubuntu 26.04 LTS + [X.org][],
  [dwm][], [suckless][] tools, [ARandR][], and [xrdp][].
* `ubuntu-26.04-lxqt.pkr.hcl` - Ubuntu 26.04 LTS + [LXQt][]
* `ubuntu-26.04-xfce.pkr.hcl` - Ubuntu 26.04 LTS + [Xfce][]
...
While `ubuntu-26.04-*.pkr.hcl` templates use `ubuntu-26.04-live-server-amd64.iso`
ISO image to create amd64 boxes by default, using `vars-ubuntu-26.04-arm64.pkrvars.hcl`
creates arm64 boxes with `ubuntu-26.04-live-server-arm64.iso` ISO
image:

    packer build -var-file=vars-ubuntu-26.04-arm64.pkrvars.hcl ubuntu-26.04-minimal.pkr.hcl
...
* `boot_wait` - Override `boot_wait` default setting, which is `4s`.
...
* `esxi_guest_os_type` - Guest OS type of ESXi box.  Defaults to
  `ubuntu-64`.  Change to `other5xlinux-64` or `other5xlinux` if you
...
* `mem_size` - RAM size of the created box.  Defaults to `2048` for
  minimal variant; or `4096` for `dwm`; or `6144` for `lxqt` and `xfce`
  variants.
