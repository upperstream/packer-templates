...
variable "iso_checksum" {
  type        = string
  default     = "sha256:2750e9a5a5d186306d6257a95df2bc0b793ff880fab2a4c89a13b64c698c573a"
...
variable "iso_path" {
  type        = string
  default     = "buster/main/installer-amd64/20190702+deb10u13/images/netboot"
...
locals {
  ...
  vm_name = coalesce(var.vm_name, "Debian-10-${var.cpu}-ansible")
  vmware_vmx_data = {
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      ...
      "WGET=wget -O -",
      "PYTHON_PIP=python3-pip=18.1-5",
      "ANSIBLE=ansible=2.7.7+dfsg-1+deb10u1",
      "ANSIBLE_LINT=ansible-lint=4.1.0+dfsg.1-1",
      "TESTINFRA_VERSION=testinfra==4.1.0",
      "PIP_COMMAND=pip3"
    ]
    scripts = [
      "../provisioners/base_debian11+.sh",
      "../provisioners/vagrant.sh",
      "../provisioners/ansible+testinfra_debian10.4.sh"
    ]
  }
  ...
  provisioner "shell" {
    environment_vars = [
      "OPEN_VM_TOOLS=open-vm-tools=2:10.3.10-1+deb10u3"
    ]
    only = [
      "vmware-iso.default",
      "vmware-iso.esxi"
