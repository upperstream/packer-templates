...
variable "iso_checksum" {
  type        = string
  default     = "file:https://mirror.leaseweb.com/devuan/devuan_daedalus/installer-iso/SHA256SUMS.txt"
  description = "SHA256 checksum of the install media."
}

variable "iso_name" {
  type        = string
  default     = "devuan_daedalus_5.0.1_amd64_server.iso"
...
locals {
  ...
  vm_name = coalesce(var.vm_name, "Devuan-5-${var.cpu}-${var.desktop}")
  vmware_vmx_data = {
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      "OPEN_VM_TOOLS_DESKTOP=open-vm-tools-desktop=2:12.2.0-1+deb12u2",
      "VMWARE_WITH_XORG=1",
