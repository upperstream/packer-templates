...
locals {
  ...
  vm_name = coalesce(var.vm_name, "Devuan-3-${var.cpu}-${var.desktop}")
  vmware_vmx_data = {
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      "OPEN_VM_TOOLS_DESKTOP=open-vm-tools-desktop=2:10.3.10-1+deb10u6",
      "VMWARE_WITH_XORG=1",
      "XSERVER_XROG_INPUT_VMMOUSE=xserver-xorg-input-evdev=1:2.10.6-1",
