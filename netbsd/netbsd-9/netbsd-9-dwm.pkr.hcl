...
variable "iso_checksum" {
  type    = string
  default = "file:https://cdn.netbsd.org/pub/NetBSD/NetBSD-9.4/iso/SHA512"
}

variable "iso_file_name" {
  type    = string
  default = "NetBSD-9.4-amd64.iso"
}

variable "iso_path" {
  type    = string
  default = "NetBSD/NetBSD-9.4/images"
}
...
locals {
  boot_command_common = [
    ...
    "${local.selector_manual_pages[var.arch]}",                 # Distribution sets - Manual pages - h (i386: g)
    "${local.selector_text_processors[var.arch]}",              # Distribution sets - Text processing tools - l (i386: k)
    "${local.selector_x11[var.arch]}",                          # Distribution sets - X11 sets - m (i386: l)
    ...
    "a<enter><wait>",                                           # Change root password - Yes
